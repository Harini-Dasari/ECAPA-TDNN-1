'''
ECAPAModel_v2.py — Trainer/evaluator for Method 1 (ECAPA_TDNN_A).

Changes vs. ECAPAModel.py:
  1. Imports ECAPA_TDNN_A from training_v2.model_A instead of ECAPA_TDNN from model
  2. CSV outputs are written to training_v2/exps_modelA/ instead of exps/
'''
...
from training_v2.model_A import ECAPA_TDNN_A


class ECAPAModel_v2(nn.Module):
    def __init__(self, lr, lr_decay, C, n_class, m, s, test_step, save_path='training_v2/exps_modelA', **kwargs):
        super(ECAPAModel_v2, self).__init__()
        ## ECAPA-TDNN — Method 1 (Temporal Aggregation)
        self.speaker_encoder = ECAPA_TDNN_A(C=C).cuda()
        ## Classifier
    ...
    def eval_network(self, eval_list, eval_path):
        ...
        with open(os.path.join(self.save_path, 'trial_predictions.csv'), 'w', newline='') as f:
            writer = csv.writer(f)
        ...
        with open(os.path.join(self.save_path, 'eval_metrics.csv'), 'w', newline='') as f:
            writer = csv.writer(f)

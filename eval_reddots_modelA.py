...
def main():
    os.makedirs('exps_reddots_modelA', exist_ok=True)
    
    print("Initializing ECAPA-TDNN Model A (Method 1)...")
    args = type('Args', (), {})()
    args.C = 1024
    args.m = 0.2
    args.s = 30
    ...
    print(f"\nFinal Results on RedDots (Method 1, Epoch 13):")
    print(f"EER: {EER:.4f}%")
    ...
    with open('exps_reddots_modelA/eval_metrics.csv', 'w', newline='') as f:
        writer = csv.writer(f)
    ...
    with open('exps_reddots_modelA/trial_predictions.csv', 'w', newline='') as f:
        writer = csv.writer(f)
